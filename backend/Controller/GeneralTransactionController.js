...
General.get('/getAllTransactions', verifyJWT, (req, res) => {
  const allTransactions = sequelize.query(
    'select * from storage.transactions t where t.isValid = 1'
  );
  allTransactions
...
General.get('/getOneTransaction/:id', verifyJWT, (req, res) => {
  ...
  transaction.then((data) => res.json(data)).catch((err) => res.json(err));
});
...
async function getBalance() {
  const users = await sequelize.query(
    `SELECT balance from storage.companies where id= 1`
  );
  return users;
}
...
async function getProductByType(type) {
  return await sequelize.query(
    'SELECT * FROM storage.products p WHERE p.`type`' + ` LIKE '%${type}%'`
  );
}
